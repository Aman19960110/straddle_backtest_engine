...
class BacktestEngine:
    ...
    def export_results_to_csv(self, df: pd.DataFrame):
        ...
        # Define conditions before cutoff (<= 2025-09-01)
        conditions_before = [
            (export_df['weekday'] == 'Tuesday')  & (export_df['date'] <= cutoff_date),
            (export_df['weekday'] == 'Wednesday') & (export_df['date'] <= cutoff_date),
            (export_df['weekday'] == 'Thursday')  & (export_df['date'] <= cutoff_date),
            (export_df['weekday'] == 'Friday')    & (export_df['date'] <= cutoff_date),
            (export_df['weekday'] == 'Monday')    & (export_df['date'] <= cutoff_date)
        ]
        choices_before = [2, 1, 0, 4, 3]

        # Define conditions after cutoff (>= 2025-09-01)
        conditions_after = [
            (export_df['weekday'] == 'Tuesday')  & (export_df['date'] > cutoff_date),
            (export_df['weekday'] == 'Wednesday') & (export_df['date'] > cutoff_date),
            (export_df['weekday'] == 'Thursday')  & (export_df['date'] > cutoff_date),
        ...
        choices_after = [0, 4, 3, 2, 1]

        # Merge both condition sets
        conditions = conditions_before + conditions_after
        choices = choices_before + choices_after
